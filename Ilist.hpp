...
// iterators.
template<typename T>
void operator-(int, ilist_iterator<T>) AKJ_DELETED_FUNCTION;
template<typename T>
void operator-(ilist_iterator<T>,int) AKJ_DELETED_FUNCTION;
...
class iplist : public Traits {
  ...
  iplist(const iplist &) AKJ_DELETED_FUNCTION;
  void operator=(const iplist &) AKJ_DELETED_FUNCTION;
...
public:
  ...
  bool AKJ_ATTRIBUTE_UNUSED_RESULT empty() const {
...
public:
  ...
  size_type AKJ_ATTRIBUTE_UNUSED_RESULT size() const {
...
namespace std {
  ...
  template<class Ty>
  void swap(akj::iplist<Ty> &Left, akj::iplist<Ty> &Right) {
    Left.swap(Right);
  }
}  // End 'std' extensions...
