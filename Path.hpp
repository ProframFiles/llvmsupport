//===- akjPath.h - Path Operating System Concept ------*- C++ -*-===//
//
...
class const_iterator {
  cStringRef Path;      ///< The entire path.
  cStringRef Component; ///< The current component. Not necessarily in Path.
  size_t    Position;  ///< The iterators current position within Path.

  // An end iterator has Position = Path.size() + 1.
  friend const_iterator begin(cStringRef path);
  friend const_iterator end(cStringRef path);

public:
  typedef const cStringRef value_type;
...
inline reverse_iterator rbegin(cStringRef path) {
...
inline reverse_iterator rend(cStringRef path) {
