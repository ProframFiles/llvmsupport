...
void APInt::initFromArray(cArrayRef<uint64_t> bigVal) {
...
APInt::APInt(unsigned numBits, cArrayRef<uint64_t> bigVal)
...
APInt::APInt(unsigned numbits, cStringRef Str, uint8_t radix)
  : BitWidth(numbits), VAL(0) {
  assert(BitWidth && "Bitwidth too small");
...
unsigned APInt::getBitsNeeded(cStringRef str, uint8_t radix) {
  ...
  cStringRef::iterator p = str.begin();
  ...
  APInt tmp(sufficient, cStringRef(p, slen), radix);
...
hash_code akj::hash_value(const APInt &Arg) {
  if (Arg.isSingleWord())
...
unsigned APInt::countLeadingZerosSlowCase() const {
  ...
  if (MSW)
    return akj::countLeadingZeros(MSW) - (APINT_BITS_PER_WORD - BitsInMSW);
  ...
  for (--i; i > 0u; --i) {
    if (pVal[i-1] == 0)
      ...
      Count += akj::countLeadingZeros(pVal[i-1]);
      break;
    }
  }
  return Count;
...
unsigned APInt::countTrailingZeros() const {
  if (isSingleWord())
    return std::min(unsigned(akj::countTrailingZeros(VAL)), BitWidth);
  unsigned Count = 0;
  unsigned i = 0;
  for (; i < getNumWords() && pVal[i] == 0; ++i)
    Count += APINT_BITS_PER_WORD;
  if (i < getNumWords())
    Count += akj::countTrailingZeros(pVal[i]);
...
APInt akj::APIntOps::GreatestCommonDivisor(const APInt& API1,
                                            const APInt& API2) {
  APInt A = API1, B = API2;
  while (!!B) {
    APInt T = B;
...
APInt akj::APIntOps::RoundDoubleToAPInt(double Double, unsigned width) {
  union {
    double D;
    uint64_t I;
  } T;
...
void APInt::fromString(unsigned numbits, cStringRef str, uint8_t radix) {
  ...
  for (cStringRef::iterator e = str.end(); p != e; ++p) {
...
void APInt::toString(cSmallVectorImpl<char> &Str, unsigned Radix,
...
/// inefficient method.  It is better to pass in a cSmallVector/SmallString
