//===- akjFileSystem.hpp - File System OS Concept -------*- C++ -*-===//
//
...
#include "IntrusiveRefCntPtr.hpp"
...
/// @returns errc::success if result has been successfully set, otherwise a
///          platform specific error_code.
error_code canonicalize(const Twine &path, cSmallVectorImpl<char> &result);
...
class mapped_file_region {
  mapped_file_region() AKJ_DELETED_FUNCTION;
  mapped_file_region(mapped_file_region&) AKJ_DELETED_FUNCTION;
  mapped_file_region &operator =(mapped_file_region&) AKJ_DELETED_FUNCTION;
...
namespace detail {
  ...
  error_code directory_iterator_construct(DirIterState&, cStringRef);
