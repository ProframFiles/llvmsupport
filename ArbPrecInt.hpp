...
template <typename T> class cSmallVectorImpl;
...
class APInt {
  ...
  ///
  /// cStringRef::getAsInteger is superficially similar but (1) does
  /// not assume that the string is well-formed and (2) grows the
  /// result to hold the input.
  void fromString(unsigned numBits, cStringRef str, uint8_t radix);
  ...
  void initFromArray(cArrayRef<uint64_t> array);
...
public:
  ...
  APInt(unsigned numBits, cArrayRef<uint64_t> bigVal);

  /// Equivalent to APInt(numBits, cArrayRef<uint64_t>(bigVal, numWords)), but
  ...
  APInt(unsigned numBits, cStringRef str, uint8_t radix);
  ...
  static unsigned getBitsNeeded(cStringRef str, uint8_t radix);
  ...
  unsigned countLeadingZeros() const {
    if (isSingleWord()) {
      ...
      return akj::countLeadingZeros(VAL) - unusedBits;
    }
  ...
  /// SmallString.
  void toString(cSmallVectorImpl<char> &Str, unsigned Radix, bool Signed,
                bool formatAsCLiteral = false) const;

  /// Considers the APInt to be unsigned and converts it into a string in the
  /// radix given. The radix can be 2, 8, 10 16, or 36.
  void toStringUnsigned(cSmallVectorImpl<char> &Str, unsigned Radix = 10) const {
    toString(Str, Radix, false, false);
  }

  /// Considers the APInt to be signed and converts it into a string in the
  /// radix given. The radix can be 2, 8, 10, 16, or 36.
  void toStringSigned(cSmallVectorImpl<char> &Str, unsigned Radix = 10) const {
  ...
  /// cSmallVector/SmallString to the methods above to avoid thrashing the heap
  /// for the string.
