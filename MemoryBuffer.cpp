...
/// CopyStringRef - Copies contents of a cStringRef into a block of memory and
/// null-terminates it.
static void CopyStringRef(char *Memory, cStringRef Data) {
...
struct NamedBufferAlloc {
  cStringRef Name;
  NamedBufferAlloc(cStringRef Name) : Name(Name) {}
};
}
...
public:
  MemoryBufferMem(cStringRef InputData, bool RequiresNullTerminator) {
    init(InputData.begin(), InputData.end(), RequiresNullTerminator);
  }

  virtual const char *getBufferIdentifier() const AKJ_OVERRIDE {
     // The name is stored after the class itself.
    return reinterpret_cast<const char*>(this + 1);
  }

  virtual BufferKind getBufferKind() const AKJ_OVERRIDE {
...
MemoryBuffer *MemoryBuffer::getMemBuffer(cStringRef InputData,
...
/// on EndPtr[0].
MemoryBuffer *MemoryBuffer::getMemBufferCopy(cStringRef InputData,
                                             cStringRef BufferName) {
...
MemoryBuffer *MemoryBuffer::getNewUninitMemBuffer(size_t Size,
  ...
  return new (Mem) MemoryBufferMem(cStringRef(Buf, Size), true);
}
...
MemoryBuffer *MemoryBuffer::getNewMemBuffer(size_t Size, cStringRef BufferName) {
...
error_code MemoryBuffer::getFileOrSTDIN(cStringRef Filename,
...
error_code MemoryBuffer::getFile(const Twine& Filename,
                                 OwningPtr<MemoryBuffer> &result,
                                 int64_t FileSize,
                                 bool RequiresNullTerminator) {
  // Ensure the path is null terminated.
  cSmallVector<char, 256> PathBuf;
	cStringRef nt_filename = Filename.toNullTerminatedStringRef(PathBuf);
  return MemoryBuffer::getFile(nt_filename.data(), result, FileSize,
                               RequiresNullTerminator);
}


error_code MemoryBuffer::getFile(cStringRef Filename,
                                 OwningPtr<MemoryBuffer> &result,
                                 int64_t FileSize,
                                 bool RequiresNullTerminator) {
  // Ensure the path is null terminated.
...
static error_code getOpenFileImpl(int FD, const char *Filename,
  ...
  while (BytesLeft) {
#ifdef HAVE_PREAD
    size_t NumRead = ::pread(FD, BufPtr, BytesLeft, MapSize-BytesLeft+Offset);
#else
    size_t NumRead = ::read(FD, BufPtr, BytesLeft);
#endif
    if (NumRead == -1) {
      if (errno == EINTR)
        continue;
