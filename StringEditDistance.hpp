...
template<typename T>
unsigned ComputeEditDistance(cArrayRef<T> FromArray, cArrayRef<T> ToArray,
  ...
  typename cArrayRef<T>::size_type m = FromArray.size();
  typename cArrayRef<T>::size_type n = ToArray.size();

  const unsigned SmallBufferSize = 64;
  unsigned SmallBuffer[SmallBufferSize];
  akj::OwningArrayPtr<unsigned> Allocated;
  ...
  for (typename cArrayRef<T>::size_type y = 1; y <= m; ++y) {
    Current[0] = y;
    unsigned BestThisRow = Current[0];

    for (typename cArrayRef<T>::size_type x = 1; x <= n; ++x) {
