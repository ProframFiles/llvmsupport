...
public:
  ...
  virtual MemSlab *Allocate(size_t Size) AKJ_OVERRIDE;
  virtual void Deallocate(MemSlab *Slab) AKJ_OVERRIDE;
};
...
class BumpPtrAllocator {
  BumpPtrAllocator(const BumpPtrAllocator &) AKJ_DELETED_FUNCTION;
  void operator=(const BumpPtrAllocator &) AKJ_DELETED_FUNCTION;
...
inline void *operator new(size_t Size, akj::BumpPtrAllocator &Allocator) {
  struct S {
    char c;
    union {
      double D;
  ...
  return Allocator.Allocate(Size, std::min((size_t)akj::NextPowerOf2(Size),
                                           offsetof(S, x)));
}

inline void operator delete(void *, akj::BumpPtrAllocator &) {}
