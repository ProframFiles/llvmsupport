//===--- akjRawOstream.hpp - Raw output stream ------------------*- C++ -*-===//
//
...
class raw_ostream {
private:
  void operator=(const raw_ostream &) AKJ_DELETED_FUNCTION;
  raw_ostream(const raw_ostream &) AKJ_DELETED_FUNCTION;
...
public:
  ...
  raw_ostream &operator<<(cStringRef Str) {
  ...
  raw_ostream &operator<<(const char *Str) {
    ...
    return this->operator<<(cStringRef(Str));
  }
  ...
  raw_ostream &write_escaped(cStringRef Str, bool UseHexEscapes = false);
...
class raw_fd_ostream : public raw_ostream {
  ...
  virtual void write_impl(const char *Ptr, size_t Size) AKJ_OVERRIDE;

  /// current_pos - Return the current position within the stream, not
  /// counting the bytes currently in the buffer.
  virtual uint64_t current_pos() const AKJ_OVERRIDE { return pos; }

  /// preferred_buffer_size - Determine an efficient buffer size.
  virtual size_t preferred_buffer_size() const AKJ_OVERRIDE;

  /// error_detected - Set the flag indicating that an output error has
  /// been encountered.
  void error_detected() { Error = true; }

	raw_fd_ostream(const char *Filename, error_code &ErrorInfo,
...
public:
  ...
  raw_fd_ostream(const char *Filename, std::string &ErrorInfo,
	...
	static error_code Create(const Twine& filename, 
  ...
  virtual raw_ostream &changeColor(enum Colors colors, bool bold=false,
                                   bool bg=false) AKJ_OVERRIDE;
  virtual raw_ostream &resetColor() AKJ_OVERRIDE;

  virtual raw_ostream &reverseColor() AKJ_OVERRIDE;

  virtual bool is_displayed() const AKJ_OVERRIDE;

  virtual bool has_colors() const AKJ_OVERRIDE;
...
class raw_string_ostream : public raw_ostream {
  ...
  /// current_pos - Return the current position within the stream, not
  /// counting the bytes currently in the buffer.
  virtual uint64_t current_pos() const AKJ_OVERRIDE { return OS.size(); }
public:
...
class raw_svector_ostream : public raw_ostream {
  ...
  /// current_pos - Return the current position within the stream, not
  /// counting the bytes currently in the buffer.
  virtual uint64_t current_pos() const AKJ_OVERRIDE;
public:
  /// Construct a new raw_svector_ostream.
  ///
  /// \param O The vector to write to; this should generally have at least 128
  /// bytes free to avoid any extraneous memory overhead.
  explicit raw_svector_ostream(cSmallVectorImpl<char> &O);
  ~raw_svector_ostream();
  ...
  /// cStringRef for the vector contents.
  cStringRef str();
};
